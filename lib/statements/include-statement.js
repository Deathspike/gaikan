...
module.exports.end = function (state, attribute) {
	...
	state.instructions.push('result.push(runtime.include(\'' + code(values.name) + '\', ' + values.data + ', outputPartials));')
...
module.exports.invoke = function (state, attribute) {
	...
	var values = extract(attribute.value);
	// Push the instruction to invoke the runtime include.
	state.instructions.push('result.push(runtime.include(\'' + code(values.name) + '\', ' + values.data + '));')
}