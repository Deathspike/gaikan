...
module.exports = function (input, root, fragments, callback, skipLayout) {
	...
	var template = module.exports.compileFromFile(input);
...
module.exports.compileFromFile = function (input) {
	...
	if (options.enableCache && typeof cache[input] !== 'undefined') {
		...
		var absolutePath = (function () {
			...
			var directories = module.exports.options.directories;
			// Initialize the extensions.
			var extensions = module.exports.options.extensions;
			// Iterate through each directory.
			for (var i = 0; i < directories.length; i += 1) {
				// Initialize the absolute path.
				var directoryPath = path.join(currentRoot, directories[i]);
				// Iterate through each extension.
				for (var j = 0; j < extensions.length; j += 1) {
					// Initialize the file name with extension.
					var fileWithExtension = input + '.' + extensions[j];
					// Initialize the file path.
					var filePath = path.join(directoryPath, fileWithExtension);
		...
		if (absolutePath) {
			...
			var template = compiler(contents, options.enableCompression);
