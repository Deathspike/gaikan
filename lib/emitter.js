 ...
 */
module.exports.code = function (state, code, isOpening) {
	// Initialize the result.
	var result = {cmd: '(' + code + ')', type: 'code'};
...
function optimize(state) {
	...
	for (var i = 0; i < state.instructions.length; i += 1) {
		...
		var isJoinable = state.instructions[i].type === 'code' || state.instructions[i].type === 'text' || state.instructions[i].type === 'variable';
		// Check if the current and next instruction are joinable.
		if (i + 1 < state.instructions.length && isJoinable && (state.instructions[i + 1].type === 'code' || state.instructions[i + 1].type === 'text' || state.instructions[i + 1].type === 'variable')) {
			// Append the next instruction command to the current command.
			state.instructions[i].cmd += ' + ' + state.instructions[i + 1].cmd;
			// Remove the next instruction command.
			state.instructions.splice(i + 1, 1);
