...
emit = function (commands, tree, token) {
	...
	for (i = 0; i < attributes.length; i += 1) {
		...
		// Initialize the key.
		var name = attribute.key.toLowerCase();
...
finalize = function (commands, enableCompression) {
	...
	for (var c = 0, n = 1; c < lines.length; c += 1, n += 1) {
	...
	for (c = 0, n = 1; c < lines.length; c += 1, n += 1) {
		// Initialize the type.
		var type = lines[c].type;
		// Determine if the command is joinable.
		var isJoinable = type === 'var' || type === 'text';
		// Check if a next command is available ...
		if (n < lines.length &&
...
find = function (token, index) {
	...
	if (token.children) {
		// Iterate through each child.
		for (var i = 0; i < token.children.length; i += 1) {
			// Find the token for the index in the child.
			var result = find(token.children[i], index);
...
push = function (commands, token) {
	...
	if (token.type === 'element') {
		...
		for (var i = 0; i < token.attributes.length; i += 1) {
			// Initialize the attribute.
			var attribute = token.attributes[i];
			// Initialize the begin quote character.
			var beginQuote = attribute.quote ? attribute.quote : '';
			// Initialize the end quote character.
			var endQuote = attribute.hasEndQuote ? beginQuote : '';
...
scan = function (commands, text, enforceAsVar) {
	...
	while ((match = regex.exec(text)) !== null) {
		...
		if (match.index !== 0) {
			...
			var splitPosition = position === -1 ? 0 : position;
			// Initialize the pre-variable text.
			var pre = text.substr(splitPosition, match.index - splitPosition);
		...
		if (type === '@') {
			...
			if ((index = name.search(/[|,]/)) !== -1) {
				...
				var splits = name.substr(index + 1).split(/[|,]/);
