...
module.exports = function (file, options, cb) {
	...
	try {
		...
		// Retrieve the in-browser state.
		var inBrowser = options._inBrowser;
		// Retrieve the locals.
		...
		// Delete the in-browser state.
		delete options._inBrowser;
		// Delete the locals.
		...
		cb(null, inBrowser ? JSON.stringify({file: path.relative(settings.path, file), root: options}) : module.exports.renderFile(options, null, file, null, cache));
	}
	// Catch an error.
	catch (err) {
		// Invoke the callback.
...
/**
 * Attach to an express application.
 */
module.exports.attach = function (app, address) {
	// Use this middleware.
	app.use(function (req, res, next) {
		// Retrieve the render function.
		var render = res.render;
		// Set the render function.
		res.render = function (name, options, cb) {
			// Check if the options is the callback.
			if (typeof options === 'function') {
				// Set the callback.
				cb = options;
				// Set the options.
			...
			// Otherwise check if options is undefined.
			else if (typeof options === 'undefined') {
				// Set the options.
				options = {};
			}
			// Check if this is an AJAX request with in-browser rendering.
			if (req.xhr && req.get('X-Gaikan-Browser')) {
				// Set the in-browser rendering state.
				options._inBrowser = true;
			}
			// Invoke the render function.
			render.call(this, name, options, cb);
		};
		// Continue.
		next();
	});
	...
	if (address) {
		...
		// Use the client-side middleware.
		app.get(address, function (req, res) {
			// Set the mime type.
			res.type('application/javascript');
			// Send the build templates.
			res.send(build);
		});
	}
};
 ...
 * Build the templates.
 */
module.exports.build = function (partial, isNotRoot) {
	// Initialize the directories.
	var directory = partial ? [partial] : (Array.isArray(module.exports.options.directory) ? module.exports.options.directory : [module.exports.options.directory]);
	// Initialize the build templates.
	var templates = '';
	...
	for (var i = 0; i < directory.length; i += 1) {
		...
		if (fs.existsSync(directory[i])) {
			...
			for (var j = 0; j < files.length; j += 1) {
				// Initialize the file.
				var file = path.join(directory[i], files[j]);
				// Check if the file is a directory.
				if (fs.statSync(file).isDirectory()) {
					// Append the templates from the directory.
					templates += module.exports.build(file, true);
				}
				// Otherwise check if the file contains the template extension and is not the layout.
				else if (file !== layout && path.extname(files[j]) === '.' + module.exports.options.extension) {
	...
	return !isNotRoot ? fs.readFileSync(__dirname + '/client.js', 'utf8').replace('/*TEMPLATES*/', templates.substr(1)) : templates;
};
