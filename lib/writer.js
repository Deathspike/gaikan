...
Writer.prototype.begin = function (attributes) {
	...
	for (i = 0; i < attributes.length; i += 1) {
		...
		case 'data-each':
			...
			var variable = 'v' + this.variableDepth + (attributes[i].value !== 'this' ? '.' + attributes[i].value : '');
			...
			this.instructions.push('for(var ' + iterator + ' in ' + variable + '){');
			...
			this.instructions.push('if (' + variable + '.hasOwnProperty(' + iterator + ')&&e.validate(v' + this.variableDepth + ')) {');
		...
		case 'data-if':
			...
			// Push the instruction to open an if statement.
			this.instructions.push('if(e.validate(' + (isNegative ? '!' : '') + 'v' + this.variableDepth + (ifValue !== 'this' ? '.' + ifValue : '') + ')){');
			// Push the attribute to be deleted.
			attributesToDelete.push(attributes[i].key);
			// Break from iteration.
			break;
		...
		case 'data-partial':
			...
			if (this.partialDepth !== 0) {
				...
				this.instructions.push('op' + this.includeDepth + '[\'' + properEscapeOfText(attributes[i].value) + '\']=function(){');
...
Writer.prototype.end = function (attributes) {
	...
	for (var i = 0; i < attributes.length; i += 1) {
		...
		case 'data-include':
			...
			this.instructions.push('r.push(e.include(\'' + properEscapeOfText(attributes[i].value) + '\',v' + this.variableDepth + ',op' + this.includeDepth + '));');
		...
		case 'data-partial':
			...
			if (this.partialDepth !== 0) {
				...
				this.instructions.push('r.push(e.partial(\'' + properEscapeOfText(attributes[i].value) + '\',v' + this.variableDepth + ',ip));');
			}
			// Break from iteration.
			break;
		}
...
Writer.prototype.invoke = function (attributes) {
	...
	for (var i = 0; i < attributes.length; i += 1) {
		...
		case 'data-include':
			...
			this.instructions.push('r.push(e.include(\'' + properEscapeOfText(attributes[i].value) + '\',v' + this.variableDepth + '));');
			// Delete the attribute.
			delete attributes[i];
			// Return true.
			return true;
...
Writer.prototype.variable = function (name, handlers, requiresEscaping, isOpeningElement) {
	...
	var variable = name === 'i' ? 'i' + this.variableDepth : 'v' + this.variableDepth + (name === 'this' ? '' : '.' + name);
...
Writer.prototype.text = function (text, isOpeningElement) {
	...
	var textToAdd = 'r.push(\'' + properEscapeOfText(text) + '\');';
...
function properEscapeOfText(text) {
	var result = '';
