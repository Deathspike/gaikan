...
var statements = {
	...
	'data-include': require('./statements/include-statement')(code, extract),
	// Include the partial statement.
	'data-partial': require('./statements/partial-statement')(code, extract),
...
module.exports.text = function (state, text, isOpening) {
	// Initialize the result.
	var result = 'result.push(\'' + code(text) + '\');';
...
module.exports.variable = function (state, name, handlers, escapeHtml, isOpening) {
	...
	if (escapeHtml) {
		// Push the html handler to the handlers.
		handlers.push('html');
	}
	...
	if (state.insert !== -1 && isOpening) {
		// Insert the variable.
		state.instructions.splice(state.insert, 0, 'result.push(' + result + ');');
		// Increment the insert location.
		state.insert += 1;
	}
	// Otherwise it is a regular variable.
		...
		// Push the variable.
		state.instructions.push('result.push(' + result + ');');
	}
};
...
/**
 * Prepare text to be embedded in a code segment.
 *
 * @param text The text.
