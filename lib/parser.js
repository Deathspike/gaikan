...
function clean(template) {
	...
	template = template.replace(/>\s+<((?!(a|abbr|acronym|b|bdo|big|br|cite|code|dfn|em|i|img|input|kbd|label|q|samp|select|small|span|strong|sub|sup|textarea|tt|var|button|del|ins|map|object|script)).)/gi, '><$1');
	// Return the template.
	return template;
}
