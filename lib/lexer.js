			...
			for (var j = 0; j <= beginnings[i][2].length; j += 1) {
				...
				if (j === beginnings[i][2].length || (quotes === null && (character === ' ' || character === '\t'))) {
					...
					if ((attribute = beginnings[i][2].substr(previousPosition, j - previousPosition).trim()) && attribute.length) {
						...
						if ((index = attribute.indexOf('=')) === -1 || attribute[0] === '@' || attribute[0] === '!' || attribute[0] === '#') {
							// Add the attribute without an undefined value.
							beginnings[i].attributes.push({ key: attribute.trim(), value: undefined });
						}
						//Otherwise the equal sign does exist.
						else {
							// Add the attribute with a value that has been stripped from quotes.
							beginnings[i].attributes.push({ key: attribute.substr(0, index).trim(), value: attribute.substr(index + 1).trim().replace(/^("|')/gi, '').replace(/("|')$/gi, '') });
						}
					}
					// Set the previous position.
					previousPosition = j;
				}
				// Otherwise check if the character is a non-escaped quote or part of a variable or code snippet.
				else if (previousCharacter !== '\\' && (character === '\'' || character === '"' || (character === '{' && (previousCharacter === '@' || previousCharacter === '!' || previousCharacter === '#')) || character === '}')) {
					// Set the character.
